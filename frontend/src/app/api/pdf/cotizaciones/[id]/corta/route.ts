...
import { CotizacionCortaDocument } from "@/lib/pdf/CotizacionCortaDocument";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const buffer = await renderToBuffer(
    createElement(CotizacionCortaDocument, { cotizacion, emisor }) as ReactElement<DocumentProps>
  );

  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `${preview ? "inline" : "attachment"}; filename="${cotizacion.numero}-corta.pdf"`,
    },
  });
}
